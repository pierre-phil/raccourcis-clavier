export const vsc = [
  {
    action: "recherche dans page en cours",
    shortcut: "ctrl + F",
  },

  {
    action: "recherche globale",
    shortcut: "ctrl + maj + F",
  },
  {
    action: "changer occurrence en cours + suivante",
    shortcut: "ctrl + D",
  },
  { action: "changer toutes les occurrences", shortcut: "ctrl + F2" },
  { action: "supprimer ligne", shortcut: "maj + suppr" },
  {
    action: "commenter le code (ligne unique)",
    shortcut: "sélectionner puis ctrl + maj + /",
  },
  {
    action: "commenter le code (multi-ligne)",
    shortcut: "sélectionner puis ctrl + maj + a",
  },
  {
    action: "voir où une fonction/variable est définie",
    shortcut: "ctrl + maj + clic",
  },
  {
    action: "supprimer une erreur",
    shortcut: "problems/clic droit/ignorer",
  },
  { action: "curseurs multiple", shortcut: "alt + clic" },
  { action: "faire monter/descende sélection", shortcut: "alt + ↑/↓" },
  { action: "afficher/masquer terminal", shortcut: "ctrl + J" },
  { action: "chercher et remplacer", shortcut: "ctrl + H" },
];
